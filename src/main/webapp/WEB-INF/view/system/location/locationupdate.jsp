...
<title>亚信一体机管理系统</title>
	...
	<script type="text/javascript">
		...
		$(function(){
			// 初始化数据
			$('#location_update_form').form('load', {
				"location.id": '<s:property value="location.id"/>',
				"location.name": '<s:property value="location.name"/>',
				"location.description": '<s:property value="location.description"/>',
				"location.datacenterId": '<s:property value="location.datacenterId"/>',
				"location.site": '<s:property value="location.site"/>',
				"location.floor": '<s:property value="location.floor"/>',
				"location.room": '<s:property value="location.room"/>',
			});
			
			// 取消
			$('#location_op_update_cancel').click(function(){
				window.location.href = '<%=ctp %>/system/location/main.action';
			});
			
			// 确定
			$('#location_op_update_submit').click(function(){
				if($('#location_update_form').form('validate')) {
					$.messager.progress({text: '正在处理，请稍后...'});
					$('#location_update_form').ajaxSubmit({
						url: '<%=ctp %>/system/location/updatelocation.action',
						type: 'POST',
						method: 'POST',
						dataType: 'json',
						success: function(data){
							$.messager.progress('close');
							$.messager.alert('成功','修改机房成功！','info',function(){
								window.location.href = '<%=ctp %>/system/location/main.action';
							});
						}
					});
	...
	<div data-options="region:'center', border: false" style="padding: 10px">
		<form id="location_update_form">
			<div class="form_fieldset">
				...
				<input type="hidden" name="location.datacenterId">
			</div>
		</form>	
		<div style="margin-left: 50px;">
			<a id="location_op_update_submit" href="javascript: void(0)" class="easyui-linkbutton" style="width: 60px;">确定</a>  
			<a id="location_op_update_cancel" href="javascript: void(0)" class="easyui-linkbutton" style="width: 60px;">取消</a>
		</div>
	</div>
</body>
</html>