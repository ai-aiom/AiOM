...
<title>亚信一体机管理系统</title>
	...
	<script type="text/javascript">
		...
		$(function(){
			...
			// 取消
			$('#machine_op_create_cancel').click(function(){
				window.location.href = '<%=ctp %>/system/machine/main.action';
			});
			
			// 确定
			$('#machine_op_create_submit').click(function(){
				if($('#machine_create_form').form('validate')) {
					$.messager.progress({text: '正在处理，请稍后...'});
					$('#machine_create_form').ajaxSubmit({
						url: '<%=ctp %>/system/machine/addmachine.action',
						type: 'POST',
						method: 'POST',
						dataType: 'json',
						success: function(data){
							$.messager.progress('close');
							$.messager.alert('成功','添加一体机成功！','info',function(){
								window.location.href = '<%=ctp %>/system/machine/main.action';
							});
						}
					});
...
<body class="easyui-layout">
	...
	<div data-options="region:'center', border: false" style="padding: 10px">
		...
		<div style="margin-left: 50px;">
			<a id="machine_op_create_submit" href="javascript: void(0)" class="easyui-linkbutton" style="width: 60px;">确定</a>  
			<a id="machine_op_create_cancel" href="javascript: void(0)" class="easyui-linkbutton" style="width: 60px;">取消</a>
		</div>
	</div>
</body>
</html>