...
<title>亚信一体机管理系统</title>
	...
	<script type="text/javascript">
		...
		$(function(){
			...
			// 取消
			$('#cabinet_op_create_cancel').click(function(){
				window.location.href = '<%=ctp %>/system/cabinet/main.action';
			});
			
			// 确定
			$('#cabinet_op_create_submit').click(function(){
				if($('#cabinet_create_form').form('validate')) {
					$.messager.progress({text: '正在处理，请稍后...'});
					$('#cabinet_create_form').ajaxSubmit({
						url: '<%=ctp %>/system/cabinet/addcabinet.action',
						type: 'POST',
						method: 'POST',
						dataType: 'json',
						success: function(data){
							$.messager.progress('close');
							$.messager.alert('成功','添加机柜成功！','info',function(){
								window.location.href = '<%=ctp %>/system/cabinet/main.action';
							});
						}
					});
...
<body class="easyui-layout">
	...
	<div data-options="region:'center', border: false" style="padding: 10px">
		<form id="cabinet_create_form">
			<div class="form_fieldset">
				<table>
					...
					<tr>
						<td>所属机房</td>
						<td><input id="select_location" class="easyui-textbox" data-options="required: true,editable:false" name="cabinet.locationId" style="width: 300px; height: 25px;"></td>
					</tr>
					<tr>
						<td>大小</td>
		...
		<div style="margin-left: 50px;">
			<a id="cabinet_op_create_submit" href="javascript: void(0)" class="easyui-linkbutton" style="width: 60px;">确定</a>  
			<a id="cabinet_op_create_cancel" href="javascript: void(0)" class="easyui-linkbutton" style="width: 60px;">取消</a>
		</div>
	</div>
</body>
</html>