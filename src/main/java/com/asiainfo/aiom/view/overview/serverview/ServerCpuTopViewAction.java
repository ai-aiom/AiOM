...
public class ServerCpuTopViewAction extends ServletAwareActionSupport
{
	...
	public String execute()
	{
		...
		Collections.sort(servers, new Comparator<Server>()
		{
			...
			{
				if(server1.getServerRuntime().getMetrics() == null || server1.getServerRuntime().getMetrics().containsKey("cpu_adle"))
				{
					return 1;
				} 
				else if(server2.getServerRuntime().getMetrics() == null || server1.getServerRuntime().getMetrics().containsKey("cpu_adle"))
				{
					return -1;
				}
				else
				{
					float cpuIdle1 = NumberUtils.toFloat(String.valueOf(server1.getServerRuntime().getMetrics().get("cpu_idle").getValue()));
					float cpuIdle2 = NumberUtils.toFloat(String.valueOf(server2.getServerRuntime().getMetrics().get("cpu_idle").getValue()));
